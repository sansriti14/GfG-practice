...
class Solution
{
  private:
    ...
    void findDistance(Node* node, int k, unordered_set<int> &st, int i, int x) {
        if(node == NULL) return;
        
        if(isLeaf(node) && i == k)
            st.insert(x);
        
        findDistance(node->left, k, st, i + 1, x);
        findDistance(node->right, k, st, i + 1, x);
     }
  ...
  public:
    int printKDistantfromLeaf(Node* root, int k)
    {
        ...
        if (root == NULL) return st.size();
    
        findDistance(root, k, st, 0, root->data);
        
        int leftResult = printKDistantfromLeaf(root->left, k);
        int rightResult = printKDistantfromLeaf(root->right, k);
    
        return st.size() + leftResult + rightResult;
    }
